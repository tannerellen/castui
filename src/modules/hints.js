...
const hints = ["{bold}q{/bold} quit", "{bold}?{/bold} help", "{bold}/{/bold} filter"];
...
const helpTextLines = [
  ...
  "",
  "{bold}Videos{/bold}",
  "enter: preview | v: view | c: copy url | r: rename | d: delete | p: toggle permanent | u: upload | m: load more | ctrl+r: refresh",
  "",
  "{bold}Filter{/bold}",
  "/: filter | esc: clear filter | enter: commit",
  "",
...
export function showHelp() {
  ...
  help = textBox({
    ...
    left: 3,
    right: 3,
    height: Math.min(20, /** @type {number} */ (screen.height) - 2),
    padding: {
      top: 1,
      right: 1,
      bottom: 1,
